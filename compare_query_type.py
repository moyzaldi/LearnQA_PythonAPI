...
for method in methods:
    response = requests.post('https://playground.learnqa.ru/ajax/api/compare_query_type', params={'method': f"{method}"})
...
for method in methods:
    response = requests.put('https://playground.learnqa.ru/ajax/api/compare_query_type', params={'method': f"{method}"})
    print(f"Если для PUT используем метод: {method}, то ответ: ", response.status_code, response.text)
print('******************')

for method in methods:
    response = requests.delete('https://playground.learnqa.ru/ajax/api/compare_query_type', params={'method': f"{method}"})
